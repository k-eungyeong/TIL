...
1. RAG 시스템 구성 순서 
  ...
  [ Vector DB 하는 일 ]
    ...
    * 사용자가 질문했을 때 RAG 검색이 수행되는 흐름 :
      사용자 질문 입력 → 질문을 임베딩 벡터로 변환 → Vector DB에서 유사벡터 검색 → 관련 원문 텍스트 가져오기 → LLM 전달 → 답변 생성
...
4. LangChain 모듈을 사용해 PDF 문서를 Vector DB에 저장(실습)
   ...
   - VS code 터미널에 "python pdf_to_vectordb.py" 명령시 에러나지 않는다면 DB 완성 → 디렉토리에 chroma_db 생성됐는지 확인하기
  
5. Ollama 서버시작
   - ".ollama/models/"로 이동 → "ollama serve" → 웹서버 127.0.0.1:11434  접속 후 Ollama is running 뜨면 서버 실행중
   - 또는 "ollama list"에서 사용할 LLM 이름 뜨면 실행중
   - "ollama ps" 명령시 서버가 돌아가고 있다면 정보 뜸! →  ps : 프로세스란 뜻으로, 올레마가 돌아가면서 어떠한 모델이랑 연동해서 돌아가면 내용이 뜨는 것
      * NAME ~ 만 뜨면 서버만 돌아가고 있는 것
   - "ollama pull nomic-embed-text" 후 디렉토리 확인
   - "ollama list" 시 nomic-embed-text 있나 확인
...
### 어려웠던 것
새로운 용어와, LangChain의 구조가 익숙치 않아 이를 이해하는데 있어 어려움이 있었습니다.

### 느낀 점
기본기를 탄탄하게, 아키텍처를 잘 구상할 수 있다면 코드를 바로 이해할 수 있는 부분이라고 생각합니다.
