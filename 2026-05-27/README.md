...
@tool("gugu_dan_tool")
def gugu_dan_tool(dan:int) -> str:
    """구구단을 계산하는 도구입니다.
    이용자가 구구단을 요청한 경우에는 이 도구가 사용되어야 한다. 
    파라미터 dan은 구구단의 단을 나타내는 정수입니다.
    예를 들어, 5를 입력하면 5단 구구단이 출력됩니다.  
    예시: "5단 구구단 보여줘" -> gugu_dan_tool(5)          # 이용자가 " "처럼 말하면 gugu_dan_tool(5)을 돌려 하는 것
    """
    try:
        return [f"{dan} x {i} = {dan * i}\n" for i in range(1, 10)]
    except:
        return "구구단 툴에서 오류가 발생했습니다. 입력이 정수인지 확인해주세요."
```
문제. test.csv 파일에 저장된 데이터를 기반으로 통계값을 보여주는 에이전트 도구 생성 
→ DB 검색을 요청한 경우에는 ‘”b_search_tool”을 에이전트가 사용하도록 정의
→ 평균, 합계, 최대, 최소, 갯수, 그 외의 요청은 서비스하지 않는다는 통보 
→ 이용자의 질문 내용에는 컬럼명과 통계의 타입 문자열이 포함되어야 함(그래야 AI-Agent가 db_search_tool 읽을 수 있음)
